...
class StatService:
    ...
    def get_player_stats(self, player_id: str) -> PlayerStatsData:
        ...
        for match in matches:
            match_data = MatchService(self.session).get_match(match.match_id)
        ...
        for match in matches_data:
            ...
            player_team = match.team_a if match.team_a.player_id == player_id else match.team_b
            opponent_team = match.team_b if match.team_a.player_id == player_id else match.team_a
        ...
        avg_goals_per_match = goals_scored / total_matches
        avg_score = (goals_scored / total_matches, goals_conceded / total_matches)
        ...
        for change in score_history:
            if change.match_id:
                match_service = MatchService(self.session)
                match_data = match_service.get_match(change.match_id)
        ...
        win_ratio_statdata = CarouselData(CarouselType.WIN_RATIO, value=[int(win_ratio_percentage * 100), int(draw_ratio_percentage * 100), int(loss_ratio_percentage * 100)])
        ...
        top_teammate_player_data = player_service.get_player(top_teammate.player_id)
        ...
        win_teammate_player_data = player_service.get_player(win_teammate.player_id)
        ...
        worst_teammate_player_data = player_service.get_player(worst_teammate.player_id)
        ...
        nemezis_player_data = player_service.get_player(nemezis.player_id)
        ...
        worst_rival_player_data = player_service.get_player(worst_rival.player_id)
        ...
        h2h_player_data = player_service.get_player(h2h.player_id)
    ...
    def get_h2h_value(self, player: Player, opponent_id: str) -> list[str]:
        ...
        for match in matches:
            match_data = match_service.get_match(match.match_id)
            if match_data.team_a.player_id == player.player_id:
                results.append(match_data.team_a.score)
            else:
                results.append(match_data.team_b.score) 
        return results
    ...
    def get_teammate_ref(self, player_id: str) -> Teammate_Ref:
            ...
            if not teammate_ref:
                teammate_ref = self.Teammate_Ref(player_id)
    ...
    def get_teammates(self, player: Player) -> list[Teammate_Ref]:
        ...
        for match_data in match_details:
            player_team = match_data.team_a if match_data.team_a.player_id == player.player_id else match_data.team_b
            opponent_team = match_data.team_b if match_data.team_a.player_id == player.player_id else match_data.team_a
